        ...
        try:
            ...
            for seq, chunk in enumerate(data_chunks):
                ...
                for attempt in range(CONNECT_RETRY + 1):
                    ...
                    # Parse JSON ACK (new format) or fallback to plain ACK:N
                    try:
                        ack_data = json.loads(ack.decode())
                        recv_seq = ack_data.get("seq", -1)
                        crc_ok   = ack_data.get("crc_ok")
                        sig_ok   = ack_data.get("sig_ok")
                    except Exception:
                        recv_seq = seq if ack == f"ACK:{seq}".encode() else -1
                        crc_ok = sig_ok = None
                    ...
                    if recv_seq == seq:
                        ...
                        if seq % 50 == 0 or retx > 0 or seq == n - 1:
                            ...
                            crc_s = ("crc=OK " if crc_ok else "crc=ERR ") if crc_ok is not None else ""
                            sig_s = ("sig=OK" if sig_ok else "sig=ERR") if sig_ok is not None else "sig=UNVERIFIED"
                            retx_s = f"  RETX={retx}" if retx else ""
                            print(f"  [SENDER] {seq+1:4d}/{n}  {pct:5.1f}%  "
                                  f"RTT={rtt:.1f}ms  {crc_s}{sig_s}{retx_s}")
                        break
                    else:
                        print(f"  [SENDER] seq={seq} unexpected ACK seq={recv_seq}")
                        retx += 1
...
class BundleReceiver:
    ...
    def __init__(self):
        ...
        self.bundle_id:    str              = ""
        self.last_parsed:  dict             = {}    # parsed dict from most recent got_chunk()
    ...
    def got_chunk(self, raw_chunk: bytes, verifier=None) -> int:
        """
        Store chunk, returns seq number or -1 if parse failed.
        Immediately verifies CRC and (if verifier given) Ed25519 signature.
        Stores parsed dict in self.last_parsed for the ACK handler to read.
        """
        from packet_framer import parse_chunk, _signed_data
        c = parse_chunk(raw_chunk)
        if not c or c["is_manifest"]:
            return -1

        # Immediate per-chunk signature verification (not deferred to reassembly)
        if verifier is not None:
            ...
            c["sig_ok"] = None   # unknown — no key yet

        self.last_parsed = c   # ACK handler reads this immediately after return
        seq = c["seq"]
...
async def _handle_data_stream(stream,
    ...
    try:
        ...
        while True:
            ...
            seq = receiver.got_chunk(raw, verifier)   # verifier passed for immediate sig check
            if seq >= 0:
                chunks_received += 1
                ts_recv_ns = time.time_ns()
                c = receiver.last_parsed   # dict with crc_ok, sig_ok, ts_sent_ns, packet_id

                # Rich JSON ACK — sender uses this for latency and integrity metrics
                ack_payload = json.dumps({
                    "seq":          seq,
                    "packet_id":    c.get("packet_id", f"{seq:06d}"),
                    "ts_recv_ns":   ts_recv_ns,
                    "ts_sent_ns":   c.get("ts_sent_ns", 0),
                    "crc_ok":       c.get("crc_ok", None),
                    "sig_ok":       c.get("sig_ok", None),
                }).encode()
                await send_msg(ack_stream, ack_payload)

                # Progress log every 10 chunks
                if chunks_received % 10 == 0 or chunks_received == receiver.total_chunks:
                    total = receiver.total_chunks
                    pct   = chunks_received / max(total, 1) * 100
                    rtt_ms = (ts_recv_ns - c.get("ts_sent_ns", ts_recv_ns)) / 1e6
                    pid_str = c.get("packet_id", "?")
                    crc_str = "OK" if c.get("crc_ok") else "ERR"
                    sig_val = c.get("sig_ok")
                    sig_str = "OK" if sig_val is True else ("ERR" if sig_val is False else "UNVERIFIED")
                    print(f"  [RECV] {chunks_received:4d}/{total}  {pct:5.1f}%  "
                          f"pid={pid_str}  RTT={rtt_ms:.2f}ms  "
                          f"crc={crc_str}  sig={sig_str}")
            else:
