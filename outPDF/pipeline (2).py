...
from arinc_generator  import generate_telemetry, BUFFER_SEC, PARAM_DEFS
...
from cvr_loader       import load_audio_file, audio_to_wav
from quic_transport   import send_bundle, run_receiver, TransportMetrics, _NullVerifier
...
def run_airplane(peer_addr: str,
    ...
    cvr_file = getattr(args_ns, 'cvr_file', None) if args_ns is not None else None
    cvr_rate  = getattr(args_ns, 'cvr_rate', 16000) if args_ns is not None else 16000
    cvr_bits  = getattr(args_ns, 'cvr_bits', 16) if args_ns is not None else 16
    cvr_ch    = getattr(args_ns, 'cvr_channels', 1) if args_ns is not None else 1

    if cvr_file and os.path.exists(cvr_file):
        cvr_raw, _cvr_meta = load_audio_file(
            cvr_file,
            sample_rate=cvr_rate,
            channels=cvr_ch,
            bit_depth=cvr_bits,
        )
        print(f"[PLANE] CVR loaded from file: {len(cvr_raw):,} bytes")
    else:
        if cvr_file:
            print(f"[PLANE] WARNING: CVR file not found: {cvr_file!r}")
        print(f"[PLANE] Using simulated CVR audio ({PRE_BUFFER_SEC}s)")
    ...
    # ── Build signed bundle ───────────────────────────────────────────────────
    # Load persistent private key if available; otherwise generate ephemeral one
    key_file = getattr(args_ns, 'key', PRIVATE_KEY_FILE) if args_ns is not None else PRIVATE_KEY_FILE
    if os.path.exists(key_file):
        sk      = load_private_key(key_file)
        signer  = Signer(private_key_bytes=bytes(sk))
        print(f"[PLANE] Loaded private key: {key_file}")
    else:
        signer  = Signer()
        print(f"[PLANE] WARNING: No key file found — using ephemeral key. "
              f"Run 'python keygen.py' for persistent authentication.")
    bundle  = pack_bundle(
        comp,
        flight_id=flight_id,
...
def run_ground(port: int = 8000, pubkey_hex: str = None, flight_id: str = "FLT0001"):
    ...
    if pubkey_hex:
        ...
        print(f"[GROUND] ✓ Using provided public key — AUTHENTICATED")
    elif os.path.exists(PUBLIC_KEY_FILE):
        vk = load_public_key(PUBLIC_KEY_FILE)
        verifier = Verifier(bytes(vk))
        print(f"[GROUND] ✓ Loaded {PUBLIC_KEY_FILE} — AUTHENTICATED")
        ...
        print(f"[GROUND] ✓ Loaded key from {OUTPUT_DIR}/{flight_id}_pubkey.hex — AUTHENTICATED")
    else:
        print(f"[GROUND] ⚠ WARNING: No public key found — chunk signatures NOT verified!")
        print(f"[GROUND]   Run 'python keygen.py' on airplane and copy {PUBLIC_KEY_FILE} here.")
...
def main():
    ...
    parser.add_argument("--key",         default=PRIVATE_KEY_FILE,
                        help=f"Private key file for signing (airplane mode, default {PRIVATE_KEY_FILE})")
    parser.add_argument("--cvr-file",    default=None,
                        help="Audio file for CVR (.wav / .bin / .pcm) — airplane mode")
    parser.add_argument("--cvr-rate",    type=int, default=16000,
                        help="Sample rate for raw .bin/.pcm CVR audio (default 16000)")
    parser.add_argument("--cvr-bits",    type=int, default=16,
                        help="Bit depth for raw .bin/.pcm CVR audio (default 16)")
    parser.add_argument("--cvr-channels",type=int, default=1,
                        help="Channels for raw .bin/.pcm CVR audio (default 1)")
